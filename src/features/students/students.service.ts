...
export class StudentsService {
  ...
  ): Promise<PaginatedStudentResponse<StudentSearchResult>> {
    try {
      ...
      const { students, total } = await this.studentsRepository.searchStudents(
        establishmentId,
        filters
      );
  ...
  async getStudentProfile(
    establishmentId: string,
    studentId: string
  ): Promise<StudentResponse<StudentProfile>> {
    try {
      ...
      const student = await this.studentsRepository.getStudentProfile(
        establishmentId,
        studentId
      );
    ...
    try {
      ...
      if (
        ...
        return {
          success: false,
          message:
            "Missing required fields: name, phone, and emergency contact are required",
          error: {
  ...
  /**
   * Get session roster for instructors
   */
  async getSessionRoster(
    establishmentId: string,
    sessionId: string
  ): Promise<StudentResponse<StudentRosterEntry[]>> {
    try {
      ...
      const roster = await this.studentsRepository.getSessionRoster(
        establishmentId,
        sessionId
      );
      ...
      return {
        success: true,
        data: roster,
        message: `Retrieved roster with ${roster.length} student${
          roster.length !== 1 ? "s" : ""
        }`,
      };
    } catch (error) {
  ...
  async getStudentsStats(
    establishmentId: string
  ): Promise<StudentResponse<StudentStats>> {
    try {
      this.logger.info("Getting students statistics", {
        establishmentId,
      });

      const stats = await this.studentsRepository.getStudentsStats(
        establishmentId
      );
