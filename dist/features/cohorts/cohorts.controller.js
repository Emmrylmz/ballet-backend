...
export class CohortsController {
    ...
    logger;
    db;
    constructor(cohortsService, logger, db) {
    ...
    createCohort = async (req, res) => {
        try {
            ...
            const result = await this.cohortsService.createCohort(establishmentId, cohortRequest, userId);
            if (result.success) {
    ...
    bulkEnrollStudents = async (req, res) => {
        try {
            ...
            const result = await this.cohortsService.bulkEnrollStudents(establishmentId, cohortId, studentIds, paymentType, userId);
            if (result.success) {
                res.status(200).json(result);
            }
            else {
    ...
    removeStudentFromCohort = async (req, res) => {
        try {
            ...
            if (result.success) {
                ...
                const statusCode = result.error?.code === COHORT_ERRORS.STUDENT_NOT_ENROLLED ? 404 : 400;
                res.status(statusCode).json(result);
            }
        }
        catch (error) {
    ...
    generateSessions = async (req, res) => {
        try {
            ...
            const result = await this.cohortsService.generateCohortSessions(establishmentId, cohortId, options, userId);
            if (result.success) {
                res.status(201).json(result);
            }
            else {
                const statusCode = result.error?.code === COHORT_ERRORS.COHORT_NOT_FOUND ? 404 : 400;
                res.status(statusCode).json(result);
            }
        }
        catch (error) {
    ...
    cloneCohort = async (req, res) => {
        try {
            ...
            const result = await this.cohortsService.cloneCohort(establishmentId, cohortId, { startDate, endDate }, userId);
            if (result.success) {
                res.status(201).json(result);
            }
            else {
