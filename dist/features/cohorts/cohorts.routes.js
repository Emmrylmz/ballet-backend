...
const createCohortSchema = Joi.object({
    ...
    scheduleDays: Joi.array().items(Joi.number().integer().min(0).max(6)).min(1).required(),
    scheduleStartTime: Joi.string().pattern(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/).required(),
    ...
    holidayBreaks: Joi.array().items(Joi.object({
    ...
    if (new Date(value.termStartDate) >= new Date(value.termEndDate)) {
        return helpers.error('any.invalid', { message: 'termStartDate must be before termEndDate' });
    }
    if (value.holidayBreaks) {
        for (const holiday of value.holidayBreaks) {
            if (new Date(holiday.start) >= new Date(holiday.end)) {
                return helpers.error('any.invalid', { message: `Holiday break "${holiday.name}": start date must be before end date` });
            }
        }
    }
    return value;
...
const updateCohortSchema = Joi.object({
    ...
    scheduleDays: Joi.array().items(Joi.number().integer().min(0).max(6)).min(1).optional(),
    scheduleStartTime: Joi.string().pattern(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/).optional(),
    ...
    if (value.termStartDate && value.termEndDate && new Date(value.termStartDate) >= new Date(value.termEndDate)) {
        return helpers.error('any.invalid', { message: 'termStartDate must be before termEndDate' });
    }
    if (value.holidayBreaks) {
        for (const holiday of value.holidayBreaks) {
...
const cloneCohortSchema = Joi.object({
    ...
    newHolidayBreaks: Joi.array().items(Joi.object({
        start: Joi.string().isoDate().required(),
        end: Joi.string().isoDate().required(),
        name: Joi.string().min(1).max(100).required()
    })).optional()
    ...
    if (new Date(value.newTermStartDate) >= new Date(value.newTermEndDate)) {
        return helpers.error('any.invalid', { message: 'newTermStartDate must be before newTermEndDate' });
    }
...
const cohortFiltersSchema = Joi.object({
    ...
    ageMin: Joi.number().integer().min(0).max(100).optional(),
    ageMax: Joi.number().integer().min(0).max(100).optional(),
    scheduleDays: Joi.alternatives().try(Joi.number().integer().min(0).max(6), Joi.string().pattern(/^[0-6](,[0-6])*$/)).optional(),
