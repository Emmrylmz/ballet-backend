export class PaymentsRepository {
    ...
    async updatePayment(establishmentId, paymentId, updateData) {
        const updates = [];
        const values = [paymentId, establishmentId];
        let paramCount = 2;
    ...
    async refundPayment(establishmentId, paymentId, refundData) {
        const query = `
      UPDATE payments
      SET
        is_refunded = true,
    ...
    async deletePayment(establishmentId, paymentId) {
    ...
    async createAssignment(establishmentId, assignmentData) {
        const query = `
    ...
    async getAssignmentsByPlan(establishmentId, paymentPlanId) {
      ...
      LEFT JOIN students s ON ppa.target_type = 'student' AND ppa.target_id = s.id::text
      LEFT JOIN cohorts c ON ppa.target_type = 'cohort' AND ppa.target_id = c.id::text
    ...
    `;
        ...
        return {
            ...
            paymentTypes: typeResult.rows.map((row) => ({
                type: row.type,
                total: parseFloat(row.total),
                count: parseInt(row.count)
            })),
    ...
    async getStudentPaymentSummaries(establishmentId, filters) {
      ...
      WITH student_payment_data AS (
        SELECT
          ...
          CASE
            WHEN sp.next_due_date IS NULL THEN 0
            WHEN sp.next_due_date < CURRENT_DATE THEN
              EXTRACT(DAY FROM (CURRENT_DATE - sp.next_due_date)) * sp.price / 30
            ELSE 0
          END as overdue_amount,
          CASE
            WHEN sp.next_due_date IS NULL THEN 0
            WHEN sp.next_due_date >= CURRENT_DATE THEN sp.price
            ELSE 0
          END as total_owed
        FROM students s
        LEFT JOIN student_packages sp ON s.id = sp.student_id AND sp.is_active = true
        LEFT JOIN payments p ON s.id = p.student_id AND p.establishment_id = $1
        WHERE s.establishment_id = $1
        GROUP BY s.id, s.name, s.email, s.phone, s.is_active, sp.package_type,
                 sp.remaining_classes, sp.next_due_date, sp.price
      )
      SELECT
        student_id,
        student_name,
        student_email,
        phone,
        COALESCE(package_type, 'none') as package_type,
        remaining_classes,
        last_payment_date,
        last_payment_amount,
        next_due_date,
        CASE
          WHEN next_due_date IS NULL THEN 'current'
          WHEN next_due_date < CURRENT_DATE THEN 'overdue'
          WHEN next_due_date <= CURRENT_DATE + INTERVAL '7 days' THEN 'due'
          ELSE 'current'
